...
type
  ...
  CacheLine : record
    state : StateType;
    data  : 0..1;  -- Reduced to 2 values for smaller state space
  end;
  ...
  Bus : record
    req     : BusReqType;
    source  : CacheId;
    data    : -1..1;  -- Reduced to match data range
    ctrl    : BusCtrlType;
  end;
...
rule "Process bus request"
  ...
  begin
    ...
    if bus.req = BusRd then
      ...
      caches[bus.source].line.data := bus.data;
      caches[bus.source].line.data := (caches[bus.source].line.data + 1) % 2;
