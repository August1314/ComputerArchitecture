/******************************
  Program "mesi_test.m" compiled by "Murphi Release 3.1"
...
#define PROTOCOL_NAME "mesi_test"
...
class mu_1_CacheId: public mu__byte
{
 public:
  ...
  mu_1_CacheId (char *name, int os): mu__byte(5, 6, 2, name, os) {};
  mu_1_CacheId (void): mu__byte(5, 6, 2) {};
  mu_1_CacheId (int val): mu__byte(5, 6, 2, "Parameter or function result.", 0)
    { operator=(val); };
...
class mu_1_BusReqType: public mu__byte
{
 public:
  ...
  mu_1_BusReqType (char *name, int os): mu__byte(7, 10, 3, name, os) {};
  mu_1_BusReqType (void): mu__byte(7, 10, 3) {};
  mu_1_BusReqType (int val): mu__byte(7, 10, 3, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
class mu_1_BusCtrlType: public mu__byte
{
 public:
  ...
  {
    if (val.defined())
      return ( s << mu_1_BusCtrlType::values[ int(val) - 11] );
    else return ( s << "Undefined" );
  };

  mu_1_BusCtrlType (char *name, int os): mu__byte(11, 13, 2, name, os) {};
  mu_1_BusCtrlType (void): mu__byte(11, 13, 2) {};
  mu_1_BusCtrlType (int val): mu__byte(11, 13, 2, "Parameter or function result.", 0)
  {
     operator=(val);
  };
    ...
    if ( ( index >= 5 ) && ( index <= 6 ) )
    ...
    else
      {
        if (index==UNDEFVAL) 
          ...
          Error.Error("Funny index value %d for %s: CacheId is internally represented from 6 to 5.\nInternal Error in Type checking.",index, name);
        return array[0];
      }
#else
    return array[ index - 5 ];
...
const int mu_BusUpgr = 10;
const int mu_CtrlNone = 11;
const int mu_CtrlFlush = 12;
const int mu_CtrlFlushOpt = 13;
...
bool Condition(unsigned r)
{
  ...
  if (r>=13 && r<=14) return R7.Condition(r-13);
  if (r>=15 && r<=16) return R8.Condition(r-15);
...
void Code(unsigned r)
{
  ...
  if (r>=13 && r<=14) { R7.Code(r-13); return; } 
  if (r>=15 && r<=16) { R8.Code(r-15); return; } 
}
int Priority(unsigned short r)
{
  ...
  if (r>=13 && r<=14) { return R7.Priority(); } 
  if (r>=15 && r<=16) { return R8.Priority(); } 
}
char * Name(unsigned r)
{
  ...
  if (r>=13 && r<=14) return R7.Name(r-13);
  if (r>=15 && r<=16) return R8.Name(r-15);
  return NULL;
}
};
const unsigned numrules = 17;
