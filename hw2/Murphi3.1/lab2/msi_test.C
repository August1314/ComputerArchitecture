/******************************
  Program "msi_test.m" compiled by "Murphi Release 3.1"
...
class mu_1_CacheId: public mu__byte
{
 public:
  ...
  mu_1_CacheId (char *name, int os): mu__byte(4, 5, 2, name, os) {};
  mu_1_CacheId (void): mu__byte(4, 5, 2) {};
  mu_1_CacheId (int val): mu__byte(4, 5, 2, "Parameter or function result.", 0)
    { operator=(val); };
...
class mu_1_CountType: public mu__byte
{
 public:
  ...
  mu_1_CountType (char *name, int os): mu__byte(0, 2, 2, name, os) {};
  mu_1_CountType (void): mu__byte(0, 2, 2) {};
  mu_1_CountType (int val): mu__byte(0, 2, 2, "Parameter or function result.", 0)
  {
    operator=(val);
  };
...
class mu_1__type_0: public mu__byte
{
 public:
  ...
  mu_1__type_0 (char *name, int os): mu__byte(0, 15, 5, name, os) {};
  mu_1__type_0 (void): mu__byte(0, 15, 5) {};
  mu_1__type_0 (int val): mu__byte(0, 15, 5, "Parameter or function result.", 0)
  {
    operator=(val);
  };
  char * Name() { return tsprintf("%d",value()); };
...
class mu_1__type_1: public mu__byte
{
 public:
  ...
  mu_1__type_1 (char *name, int os): mu__byte(0, 15, 5, name, os) {};
  mu_1__type_1 (void): mu__byte(0, 15, 5) {};
  mu_1__type_1 (int val): mu__byte(0, 15, 5, "Parameter or function result.", 0)
  {
    operator=(val);
  };
  char * Name() { return tsprintf("%d",value()); };
...
class mu_1_BusReqType: public mu__byte
{
 public:
  ...
  {
    if (val.defined())
      return ( s << mu_1_BusReqType::values[ int(val) - 6] );
    else return ( s << "Undefined" );
  };

  mu_1_BusReqType (char *name, int os): mu__byte(6, 8, 2, name, os) {};
  mu_1_BusReqType (void): mu__byte(6, 8, 2) {};
  mu_1_BusReqType (int val): mu__byte(6, 8, 2, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
class mu_1_BusCtrlType: public mu__byte
{
 public:
  ...
  {
    if (val.defined())
      return ( s << mu_1_BusCtrlType::values[ int(val) - 9] );
    else return ( s << "Undefined" );
  };

  mu_1_BusCtrlType (char *name, int os): mu__byte(9, 10, 2, name, os) {};
  mu_1_BusCtrlType (void): mu__byte(9, 10, 2) {};
  mu_1_BusCtrlType (int val): mu__byte(9, 10, 2, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
class mu_1__type_2: public mu__long
{
 public:
  ...
  mu_1__type_2 (char *name, int os): mu__long(-1, 15, 5, name, os) {};
  mu_1__type_2 (void): mu__long(-1, 15, 5) {};
  mu_1__type_2 (int val): mu__long(-1, 15, 5, "Parameter or function result.", 0)
  {
    operator=(val);
  };
  char * Name() { return tsprintf("%d",value()); };
    ...
    if ( ( index >= 4 ) && ( index <= 5 ) )
    ...
    else
      {
        if (index==UNDEFVAL) 
          ...
          Error.Error("Funny index value %d for %s: CacheId is internally represented from 5 to 4.\nInternal Error in Type checking.",index, name);
        return array[0];
      }
#else
    return array[ index - 4 ];
...
const int mu_BusNone = 6;
const int mu_BusRd = 7;
const int mu_BusRdX = 8;
const int mu_CtrlNone = 9;
const int mu_CtrlFlush = 10;
/*** Variable declaration ***/
mu_1__type_3 mu_caches("caches",0);

/*** Variable declaration ***/
mu_1_MemBlock mu_mem("mem",32);

/*** Variable declaration ***/
mu_1_Bus mu_bus("bus",40);
...
mu_caches[mu_bus.mu_source].mu_line.mu_data = mu_bus.mu_data;
mu_caches[mu_bus.mu_source].mu_line.mu_data = ((mu_caches[mu_bus.mu_source].mu_line.mu_data) + (1)) % (16);
}
}
...
public:
  ...
  bool Condition(unsigned r)
  {
    ...
    r = r / 2;
bool mu__boolexpr8;
  if (!((mu_caches[mu_i].mu_line.mu_state) == (mu_M))) mu__boolexpr8 = FALSE ;
  else {
    ...
    while (what_rule < 7 )
      {
        if ( ( TRUE  ) ) {
bool mu__boolexpr9;
  if (!((mu_caches[mu_i].mu_line.mu_state) == (mu_M))) mu__boolexpr9 = FALSE ;
  else {
  ...
  void Code(unsigned r)
  {
    ...
    mu_i.value((r % 2) + 4);
    r = r / 2;
mu_caches[mu_i].mu_line.mu_data = ((mu_caches[mu_i].mu_line.mu_data) + (1)) % (16);
  };
...
public:
  ...
  bool Condition(unsigned r)
  {
    ...
    r = r / 2;
bool mu__boolexpr10;
  if (!((mu_caches[mu_i].mu_line.mu_state) == (mu_I))) mu__boolexpr10 = FALSE ;
  else {
    ...
    while (what_rule < 9 )
      {
        if ( ( TRUE  ) ) {
bool mu__boolexpr11;
  if (!((mu_caches[mu_i].mu_line.mu_state) == (mu_I))) mu__boolexpr11 = FALSE ;
  else {
...
public:
  ...
  bool Condition(unsigned r)
  {
    ...
    r = r / 2;
bool mu__boolexpr12;
  if (!((mu_caches[mu_i].mu_line.mu_state) == (mu_S))) mu__boolexpr12 = FALSE ;
  else {
    ...
    while (what_rule < 11 )
      {
        if ( ( TRUE  ) ) {
bool mu__boolexpr13;
  if (!((mu_caches[mu_i].mu_line.mu_state) == (mu_S))) mu__boolexpr13 = FALSE ;
  else {
...
public:
  ...
  bool Condition(unsigned r)
  {
    ...
    r = r / 2;
bool mu__boolexpr14;
  if (!((mu_caches[mu_i].mu_line.mu_state) == (mu_M))) mu__boolexpr14 = FALSE ;
  else {
    ...
    while (what_rule < 13 )
      {
        if ( ( TRUE  ) ) {
bool mu__boolexpr15;
  if (!((mu_caches[mu_i].mu_line.mu_state) == (mu_M))) mu__boolexpr15 = FALSE ;
  else {
...
bool Condition(unsigned r)
{
  ...
  if (r>=5 && r<=6) return R3.Condition(r-5);
  if (r>=7 && r<=8) return R4.Condition(r-7);
  if (r>=9 && r<=10) return R5.Condition(r-9);
  if (r>=11 && r<=12) return R6.Condition(r-11);
...
void Code(unsigned r)
{
  ...
  if (r>=5 && r<=6) { R3.Code(r-5); return; } 
  if (r>=7 && r<=8) { R4.Code(r-7); return; } 
  if (r>=9 && r<=10) { R5.Code(r-9); return; } 
  if (r>=11 && r<=12) { R6.Code(r-11); return; } 
}
int Priority(unsigned short r)
{
  ...
  if (r>=5 && r<=6) { return R3.Priority(); } 
  if (r>=7 && r<=8) { return R4.Priority(); } 
  if (r>=9 && r<=10) { return R5.Priority(); } 
  if (r>=11 && r<=12) { return R6.Priority(); } 
}
char * Name(unsigned r)
{
  ...
  if (r>=5 && r<=6) return R3.Name(r-5);
  if (r>=7 && r<=8) return R4.Name(r-7);
  if (r>=9 && r<=10) return R5.Name(r-9);
  if (r>=11 && r<=12) return R6.Name(r-11);
  return NULL;
}
};
const unsigned numrules = 13;

/********************
  parameter
 ********************/
#define RULES_IN_WORLD 13
...
/********************
  Invariant records
 ********************/
int mu__invariant_16() // Invariant "Shared Caches equal memory (when no Modified cache exists)"
{
bool mu__boolexpr17;
  if (!((mu_CountModifiedCache(  )) == (0))) mu__boolexpr17 = TRUE ;
  else {
bool mu__quant18; 
mu__quant18 = TRUE;
{
for(int mu_i = 4; mu_i <= 5; mu_i++) {
bool mu__boolexpr19;
  if (!((mu_caches[mu_i].mu_line.mu_state) == (mu_S))) mu__boolexpr19 = TRUE ;
  else {
  mu__boolexpr19 = ((mu_caches[mu_i].mu_line.mu_data) == (mu_mem.mu_data)) ; 
}
if ( !(mu__boolexpr19) )
...
bool mu__condition_20() // Condition for Rule "Shared Caches equal memory (when no Modified cache exists)"
{
...
int mu__invariant_21() // Invariant "Modified Cache implies no Shared Cache"
{
bool mu__boolexpr22;
  if (!((mu_CountModifiedCache(  )) == (1))) mu__boolexpr22 = TRUE ;
  else {
bool mu__quant23; 
mu__quant23 = TRUE;
{
...
bool mu__condition_24() // Condition for Rule "Modified Cache implies no Shared Cache"
{
...
int mu__invariant_25() // Invariant "All Shared Caches have same data"
{
bool mu__quant26; 
mu__quant26 = TRUE;
{
...
bool mu__boolexpr28;
bool mu__boolexpr29;
  if (!((mu_caches[mu_i].mu_line.mu_state) == (mu_S))) mu__boolexpr29 = FALSE ;
  else {
  mu__boolexpr29 = ((mu_caches[mu_j].mu_line.mu_state) == (mu_S)) ; 
}
  if (!(mu__boolexpr29)) mu__boolexpr28 = TRUE ;
  else {
  mu__boolexpr28 = ((mu_caches[mu_i].mu_line.mu_data) == (mu_caches[mu_j].mu_line.mu_data)) ; 
}
if ( !(mu__boolexpr28) )
...
bool mu__condition_30() // Condition for Rule "All Shared Caches have same data"
{
...
int mu__invariant_31() // Invariant "Only one Cache may be Modified"
{
return (mu_CountModifiedCache(  )) <= (1);
};

bool mu__condition_32() // Condition for Rule "Only one Cache may be Modified"
{
...
{"Only one Cache may be Modified", &mu__condition_32, NULL, FALSE},
{"All Shared Caches have same data", &mu__condition_30, NULL, FALSE},
{"Modified Cache implies no Shared Cache", &mu__condition_24, NULL, FALSE},
{"Shared Caches equal memory (when no Modified cache exists)", &mu__condition_20, NULL, FALSE},
};
...
: Presentation(Simple)
{
  ...
  if (  args->sym_alg.mode == argsym_alg::Exhaustive_Fast_Canonicalize) {
    mu_1_CacheId Perm0[2];

  /********************
   declaration of class variables
  ********************/
  in = new bool[2];
 in_mu_1_CacheId = new int[2];
 perm_mu_1_CacheId = new arr_mu_1_CacheId[2];
 revperm_mu_1_CacheId = new arr_mu_1_CacheId[2];
    ...
    for (i=0; i<2; i++)
      ...
      if (i_mu_1_CacheId >= 2) { i_mu_1_CacheId = 0; carry = 1; } 
      else { carry = 0; } 
    }
  }
  else
...
void PermSet::SimpleToExplicit()
{
  ...
  int start_mu_1_CacheId[2];
  int size_mu_1_CacheId[2];
  bool should_be_in_mu_1_CacheId[2];
...
void mu_1__type_3::Canonicalize(PermSet& Perm)
{
  ...
  bool exists;
  bool split;
  bool goodset_mu_1_CacheId[2];
  bool pos_mu_1_CacheId[2][2];
  // range mapping
  int start;
  int class_size;
  int size_mu_1_CacheId[2];
